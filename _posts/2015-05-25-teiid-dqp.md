...
BlockedException is Teiid's way to release the current thread instead of waiting for a processing resource to complete, like waiting for data from a data source. As above sequence diagram **Appendix-3**, check the FutureTask,

* If FutureTask not complete throw `BlockedException`, release current thread
* If FutureTask complete, get `AtomicResultsMessage` as result return
...
## Teiid Engine Request Process - RequestWorkItem
