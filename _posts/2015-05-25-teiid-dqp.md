...
## Teiid Engine Process

[RequestWorkItem](http://ksoong.org/teiid-uml-diagram) implements `Runnable` interface, it executed by threads in processWorkerPool. A RequestWorkItem contains a [Processor Plan](#Process of generate a ProcessorPlan), it has the following attributes:

![RequestWorkItem run]({{ site.baseurl }}/assets/blog/teiid-requestWorkItem-attr.png)

### Threadpools related with the Engine Process



**Netty Worker Pool** - the work related with Engine Process are execute DQPCore's executeRequest() method, init the `Request` and `RequestWorkItem`, then start the `RequestWorkItem` which wrapped a active processor plan.

**processWorkerPool** - hold threads that execute Runnable [RequestWorkItem](http://ksoong.org/teiid-uml-diagram), the creation looks
...
This thread pool will execute 2 kinds of threads:

* Processing Threads - [RequestWorkItem](http://ksoong.org/teiid-uml-diagram) 
* Querying Threads - A Callable task
